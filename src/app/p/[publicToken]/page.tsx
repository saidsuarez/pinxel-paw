...
import type { Pet, Profile, PublicProfileSettings, VeterinaryRecord } from "@/types";
...
export default async function PublicPetPage({ params }: { params: Promise<{ publicToken: string }> }) {
  ...
  const [{ data: settings }, { data: owner }, { data: recordsData }] = await Promise.all([
    supabase.from("public_profile_settings").select("*").eq("pet_id", pet.id).maybeSingle<PublicProfileSettings>(),
    supabase.from("profiles").select("*").eq("user_id", pet.owner_id).maybeSingle<Profile>(),
    supabase
      .from("veterinary_records")
      .select("*")
      .eq("pet_id", pet.id)
      .in("record_type", ["vacuna", "alergia"])
      ...
      <div className="mx-auto max-w-3xl">
        <section className="overflow-hidden rounded-lg border bg-card shadow-sm">
          <div className="grid gap-0 md:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="p-6">
              <Badge className="bg-primary/10 text-primary">Perfil NFC verificado</Badge>
